...
void Line::translation(IplImage* image, Tire *tire)
{
	...
	for(int i=-1; i<=1; i++)
	{
		...
		if(start.x < 0 || start.y < 0 || start.x+mask_width > 2000 || start.y+mask_width > 2000)
		{
...
void Line::startUp(IplImage *image, CvPoint start, int heading, Tire *tires)
{
	...
	for(int i=0; i<2; i++)
	{
		...
		while(result.sum < THRESHOLD)
		{
			...
			tires[i].center.y = tires[i].center.y - line_width*cos(normal*PI/180.);
			tires[i].center.x = tires[i].center.x - line_width*sin(normal*PI/180.);
			...
			tires[i].heading = tires[i].heading*0.4 + heading*0.6;
			...
			cvSetImageROI(vMap, cvRect(result.center.x - 500, result.center.y-500, 1000, 1000));
...
void Line::makeLineMap(IplImage *image, IplImage *map, CvPoint start, int heading)
{
	...
	axle.tires[LEFT].center  = axle.tires[RIGHT].center  = start;
	...
	while( dist_to_last < 125 )
	{
			...
			{	
				...
				else
				{
					...
					if( ((axle.tires[LEFT].center.x - axle.tires[RIGHT].center.x)*	
						 (axle.tires[LEFT].center.x - axle.tires[RIGHT].center.x) +
						 (axle.tires[LEFT].center.y - axle.tires[RIGHT].center.y)*
						 (axle.tires[LEFT].center.y - axle.tires[RIGHT].center.y)) < 1000 )
					{
		...
		cvSetImageROI(vMap, cvRect(axle.tires[LEFT].center.x - 500, axle.tires[RIGHT].center.y -500, 1000, 1000));
		cvShowImage("Orig", vMap);
