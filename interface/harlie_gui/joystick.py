...
class JoyGauge(wx.Panel):
    ...
    def DrawJoystick(self, dc):
        ...
        if self.stick:
            command = self.ComputeVectorCommand()
            evt = harlie_gui.gui_events.UpdateVectorCommand(speed = command[0], angle_change = command[1])
    ...
    def ComputeVectorCommand(self):
        if self.stick:
            ...
            speed = -1 * yratio * settings.joystick["max_speed"]
            angle_change = xratio * settings.joystick["max_angle"]

            return (speed, angle_change)
...
class JoystickDemoPanel(wx.Panel):
    ...
    def __init__(self, parent, log):
        ...
        self.Bind(wx.EVT_TIMER, self.OnJoystick, self.timer)
